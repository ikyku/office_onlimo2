...
export const TableView: React.FC<TableViewProps> = ({
  ...
  const getStatusBadge = (status: WaterQualityStatus) => {
    ...
    if (status === 'baku_mutu') {
      badgeBg = 'bg-emerald-100/80 text-emerald-800 dark:bg-emerald-950/70 dark:text-emerald-300 border border-emerald-300/60 dark:border-emerald-700/60';
      Icon = CheckCircle2;
    } else if (status === 'cemar_ringan') {
      badgeBg = 'bg-blue-100/80 text-blue-800 dark:bg-blue-950/70 dark:text-blue-300 border border-blue-300/60 dark:border-blue-700/60';
      Icon = CheckCircle2;
    } else if (status === 'cemar_sedang') {
      badgeBg = 'bg-amber-100/80 text-amber-800 dark:bg-amber-950/70 dark:text-amber-300 border border-amber-300/60 dark:border-amber-700/60';
      Icon = AlertCircle;
    } else if (status === 'cemar_berat') {
      badgeBg = 'bg-rose-100/80 text-rose-800 dark:bg-rose-950/70 dark:text-rose-300 border border-rose-300/60 dark:border-rose-700/60';
      Icon = AlertCircle;
    }

    return (
      <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold ${badgeBg}`}>
